...
def main() -> None:
    ...
    sc = sub.add_parser(
        "scaffold",
        help=(
            "Create/update a multi-file project scaffold from a description (writes multiple files). "
            "Useful for frameworks like Flask with routes, services, templates, etc."
        ),
    )
    sc.add_argument("--desc", required=True, help="Natural language description/spec")
    sc.add_argument(
        "--out-dir",
        default=".",
        help="Relative output directory root for generated files (default: .)",
    )
    sc.add_argument(
        "--overwrite",
        action="store_true",
